...
class PracticeLcationService
{
	...
	{
		$appointmentType = PracticeLocation::onlyTrashed()->get();
		return response()->json($appointmentType);
	}

  public function getData()
	{
		$appointmentType = PracticeLocation::where('id', '!=', 1)->get();
		return response()->json($appointmentType);
	}
	...
	{
		$check = PracticeLocation::find($request->id);
		...
		try {
			PracticeLocation::where('id', $request['id'])->update($data);
	...
	{
		$check = PracticeLocation::withTrashed()->where('id', $request->id)->get();
		...
		try {
			PracticeLocation::where('id', $request['id'])->restore();
