...
class SpecialityService
{
  ...
  {
    $appointmentType = Speciality::onlyTrashed()->get();
    return response()->json($appointmentType);
  }

  public function getData()
  {
    $appointmentType = Speciality::where('id', '!=', 1)->get();
    return response()->json($appointmentType);
  }
  ...
  {
    $check = Speciality::find($request->id);
    ...
    try {
      Speciality::where('id', $request['id'])->update($data);
  ...
  {
    $check = Speciality::withTrashed()->where('id', $request->id)->get();
    ...
    try {
      Speciality::where('id', $request['id'])->restore();
