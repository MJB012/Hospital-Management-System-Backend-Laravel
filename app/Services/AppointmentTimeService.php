...
class AppointmentTimeService
{
  ...
  {
    $appointmentType = AppointmentTime::onlyTrashed()->get();
    return response()->json($appointmentType);
  }
  public function getData()
  {
    $appointmentType = AppointmentTime::all();
    return response()->json($appointmentType);
  }
  ...
  {
    $check = AppointmentTime::find($request->id);
    ...
    try {
      AppointmentTime::where('id', $request['id'])->update($data);
  ...
  {
    $check = AppointmentTime::withTrashed()->where('id', $request->id)->get();

    if (!$check) {
      return response()->json(['status' => false, 'message' => 'Field Not found']);
    }
    try {
      AppointmentTime::where('id', $request['id'])->restore();
